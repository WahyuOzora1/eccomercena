...
class CartController extends Controller
{
    ...
    public function CheckoutCreate()
    {
        ...
        if (Auth::check()) {
            if (Cart::count() > 0) {
                $carts = Cart::content();
                $exceedsStock = false; // Flag to check if quantity exceeds stock

                $carts->each(function ($cartItem, $key) use (&$exceedsStock) {
                    $totalbeli = Cart::count();
                    $id_produk = $cartItem->id;
                    $product = Product::findOrFail($id_produk);
                    $stok = $product->product_qty;

                    if ($totalbeli > $stok) {
                        $exceedsStock = true; // Set the flag to true if quantity exceeds stock
                    }
                });

                if ($exceedsStock) {
                    $notification = array(
                        'message' => 'Produk yang dibeli melebihi stok',
                        'alert-type' => 'error'
                    );
